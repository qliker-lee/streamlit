...
def main():
    start = time.time()
    try:
        processor = CodeERDMapping()
        processor.run_erd_mapping()
        print("✅ Code ERD Mapping 완료")
        print(f"총 처리 시간: {time.time() - start:.2f}초")
    except Exception as e:
        print(f"Code ERD Mapping 프로그램 실행 실패: {e}")
        sys.exit(1)
