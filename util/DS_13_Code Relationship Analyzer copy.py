...
try:
    from util.io import load_yaml_datasense
...
# REFERENCE_MAPPING_FILE = OUTPUT_DIR / 'ReferenceMapping.csv'
# RULE_MAPPING_FILE = OUTPUT_DIR / 'RuleMapping.csv'
# NUMERIC_STATS_FILE = OUTPUT_DIR / 'NumericStats.csv'
...
class Initializing_Main_Class:
    ...
    def process_files_mapping(self):
        try:
            ...
            if df_ff.empty or df_rt.empty or df_mm.empty:
                self.logger.error(f"기본 파일 로드 실패: {f_format_path}, {r_datatype_path}, {m_meta_path}")
                return False
            ...
            if result_df is None or result_df.empty:
                self.logger.error("[execute_relationship_analysis] result_df is empty")
                return False
            
            result_df.to_csv(CODEMAPPING_FILE, index=False, encoding='utf-8-sig')
            self.logger.info(f"code mapping : {CODEMAPPING_FILE} 저장")
    ...
    def execute_relationship_analysis(self, df_ff, df_rt, df_mm) -> pd.DataFrame:
        ...
        if reference_df is None or reference_df.empty:
            self.logger.info("[reference_mapping] reference_df is empty")
            return pd.DataFrame()
        if DEBUG_MODE and reference_df is not None and not reference_df.empty:
            ...
            self.logger.info(f"3rd reference mapping : {p} 저장")

        # 2) Rule
        rule_df = self.rule_mapping(df_ff, df_rt)
        if rule_df is None or rule_df.empty:
            self.logger.info("[rule_mapping] rule_df is empty")
            return pd.DataFrame()
        if DEBUG_MODE and rule_df is not None and not rule_df.empty:
            ...
            self.logger.info(f"4th rule mapping : {p} 저장")
        ...
        if DEBUG_MODE and numeric_df is not None and not numeric_df.empty:
            ...
            self.logger.info(f"5th numeric stats : {p} 저장")
        ...
        if internal_df is None or internal_df.empty:
            self.logger.error("[internal_mapping] internal_df is empty")
            return pd.DataFrame()
        if DEBUG_MODE and internal_df is not None and not internal_df.empty:
            ...
            self.logger.info(f"6th internal mapping : {p} 저장")
        ...
        if concat_df is None or concat_df.empty:
            self.logger.error("[mapping_concat] concat_df is empty")
            return pd.DataFrame()
        if DEBUG_MODE and concat_df is not None and not concat_df.empty:
            ...
            self.logger.info(f"7th concat_df : {p} 저장")

        pivoted_df = self.mapping_pivot(internal_df) 
        if pivoted_df is None or pivoted_df.empty:
            self.logger.error("[mapping_pivot] pivoted_df is empty")
            return pd.DataFrame()
        if DEBUG_MODE and pivoted_df is not None and not pivoted_df.empty:
            ...
            self.logger.info(f"8th pivoted_df : {p} 저장")

        final_df = self.final_mapping(df_ff, pivoted_df, reference_df, rule_df) # 새로운 방식 
        if final_df is None or final_df.empty:
            self.logger.error("[final_mapping] final_df is empty")
            return pd.DataFrame()
        if DEBUG_MODE and final_df is not None and not final_df.empty:
            ...
            self.logger.info(f"9th final mapping : {p} 저장")
...
#         return out
    ...
    def rule_mapping(
        ...
        # self.logger.info(f"[rule_mapping] 시작 - ruldatatype_df: {len(ruldatatype_df) if ruldatatype_df is not None and not ruldatatype_df.empty else 0}행")
        ...
        if miss:
            self.logger.error(f"[rule_mapping] ruldatatype_df 필수 컬럼 누락: {sorted(miss)}")
            self.logger.info(f"[rule_mapping] ruldatatype_df 컬럼: {list(ruldatatype_df.columns)}")
        ...
        # self.logger.info(f"[rule_mapping] 필터링 전: {len(ruldatatype_df)}행, 필터링 후: {len(df_rule)}행")
        # self.logger.info(f"[rule_mapping] MasterType 분포: {ruldatatype_df['MasterType'].value_counts().to_dict()}")
        # self.logger.info(f"[rule_mapping] Rule이 비어있지 않은 행: {(rule_clean != '').sum()}개")
        
        if df_rule.empty:
            self.logger.warning("[rule_mapping] df_rule is empty - MasterType이 'Reference'이거나 Rule이 빈 문자열인 행만 있음")
            return pd.DataFrame(columns=out_cols)
        ...
        # self.logger.info(f"[rule_mapping] Rule 표준화 후 - df_rule: {len(df_rule)}행")
        # self.logger.info(f"[rule_mapping] 유효 Rule 타입: {sorted(vtypes)}")
        # self.logger.info(f"[rule_mapping] 표준화된 Rule 분포: {df_rule['Rule'].value_counts().to_dict()}")
        # self.logger.info(f"[rule_mapping] valid_types에 포함된 Rule: {len(rule_df)}행")
        
        if rule_df.empty:
            self.logger.warning(f"[rule_mapping] rule_df is empty - valid_types에 포함된 Rule이 없음")
            # self.logger.info(f"[rule_mapping] 표준화된 Rule 목록: {sorted(df_rule['Rule'].unique())}")
            return pd.DataFrame(columns=out_cols)
        ...
        # self.logger.info(f"[rule_mapping] 처리할 파일 그룹 수: {len(file_groups)}개")
        ...
        for fpath, grp in file_groups:
            ...
            if not os.path.exists(src_path):
                self.logger.warning(f"[rule_mapping] 파일 경로 확인 불가: {src_path} (원본: {fpath})")
                # 대체 경로 시도
                alt_paths = [
                ...
                for alt in alt_paths:
                    if os.path.exists(alt):
                        src_path = alt
                        self.logger.info(f"[rule_mapping] 대체 경로 사용: {src_path}")
                        found = True
                        break
                if not found:
                    continue
            ...
            for _, r in grp.iterrows():
                ...
                if col not in df_src.columns:
                    self.logger.debug(f"[rule_mapping] 컬럼 없음: {src_path}::{col} (파일 컬럼: {list(df_src.columns)[:5]}...)")
                    continue

                series = df_src[col].dropna().astype(str)
                non_null = len(series)
                if non_null == 0:
                    self.logger.debug(f"[rule_mapping] NULL만 있음: {src_path}::{col}")
                    continue

                key = str(r['Rule']).strip().upper()
                fn = mapper.get(key)
                if fn is None:
                    self.logger.warning(f"[rule_mapping] 미지원 Rule: {key} (지원되는 Rule: {sorted(mapper.keys())})")
                    continue

                try:
                    # apply validator and count True
                    valid_count = int(series.apply(fn).sum())
                    # self.logger.debug(f"[rule_mapping] 검증 결과: {src_path}::{col} ({key}) - 유효: {valid_count}/{non_null}")
                except Exception as e:
                    self.logger.warning(f"[rule_mapping] validate 실패: {src_path}::{col} ({key}) -> {e}")
                    continue

                if valid_count <= 0:
                    self.logger.debug(f"[rule_mapping] 유효 데이터 없음: {src_path}::{col} ({key})")
                    continue
        ...
        # self.logger.info(f"[rule_mapping] 처리 완료 - 결과 수: {len(results)}개")
        
        if not results:
            self.logger.warning("[rule_mapping] 결과가 없습니다 - 모든 검증이 실패했거나 valid_count가 0인 경우일 수 있습니다")
            return pd.DataFrame(columns=out_cols)

        out = pd.DataFrame(results)[out_cols].copy()
        self.logger.info(f"[rule_mapping] 최종 결과: {len(out)}행")
    ...
    def reference_mapping(self, fileformat_df: pd.DataFrame) -> pd.DataFrame:
        ...
        if expand_df is None or expand_df.empty:
            self.logger.error("ref_1st : [reference_mapping] expand_df is empty")
            return pd.DataFrame()
        if DEBUG_MODE and expand_df is not None and not expand_df.empty:
            ...
            self.logger.info(f"1st expand_df : {p} 저장")
        ...
        if reference_df is None or reference_df.empty:
            self.logger.error("ref_2nd : [reference_mapping] expand_df 중 Reference 타입 레코드 없음")
            return pd.DataFrame()

        combine_df = Combine_Format(source_df, reference_df)
        if combine_df is None or combine_df.empty:
            self.logger.error("ref_2nd : [reference_mapping] combine_df is empty")
            return pd.DataFrame()
        ...
        if DEBUG_MODE and combine_df is not None and not combine_df.empty:
            ...
            self.logger.info(f"2nd combine_df : {p} 저장")
        ...
        if mapping_df is None or mapping_df.empty:
            self.logger.error("ref_3rd : [reference_mapping] mapping_check_df is empty")
            return pd.DataFrame()
        ...
        if mapping_df is None or mapping_df.empty:
            self.logger.error(f"ref_4th : [reference_mapping] {MATCH_RATE_THRESHOLD} 조건 미충족")
            return pd.DataFrame()   
        return mapping_df
    ...
    def final_mapping(self, fileformat_df, pivoted_df, reference_df, rule_df) -> pd.DataFrame:
        ...
        if not df_rule.empty:
            for c in rule_required_cols:
                if c not in df_rule.columns:
                    # self.logger.warning(f"[final_mapping] rule_df에 누락된 컬럼: {c}")
                    df_rule[c] = ""
        ...
        if reference_df is None or reference_df.empty:
            ...
            if missing_cols:
                self.logger.warning(f"[final_mapping] reference_df에 누락된 컬럼: {missing_cols}")
                self.logger.info(f"[final_mapping] reference_df 컬럼: {list(reference_df.columns)}")
                # 누락된 컬럼 추가
            ...
            if ref_df.empty:
                self.logger.debug("reference_df 필터링 후 비어있음 -> 참조 반영 스킵")
...
if __name__ == "__main__":
    ...
    main_config = load_yaml_datasense()
