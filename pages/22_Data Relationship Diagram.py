# -*- coding: utf-8 -*-
"""
🔗 DataSense Logical Data Relationship Diagram Generator (Final)
--------------------------------------------------
✔ Cloud / Local 환경 자동 감지
✔ Cloud : 예제 이미지 출력
✔ Local : 실제 Graphviz Data Relationship Diagram 생성
...
# -------------------------------------------------
APP_NAME = "Data Relationship Diagram"
...
def generate_erd(selected_tables, pk_map, it_df):
    """     3rd Step: Logical Data Relationship Diagram 생성     """
    
    st.subheader("2. Logical Data Relationship Diagram 분석 결과")
    ...
    try:
        ...
        png_filename = f"{IMAGE_FILE}_{file_time}.png"
        ...
        if png_success and actual_png_filepath:
            ...
            caption = f"Data Relationship Diagram: {', '.join(selected_tables[:5])}{'...' if len(selected_tables) > 5 else ''}"
        ...
        try:
            ...
            st.warning(f"⚠️ SVG 파일 저장 실패: {e}")
            return False

    except Exception as e:
        st.error(f"❌ Data Relationship Diagram 생성 중 오류가 발생했습니다: {e}")
        return False

def display_erd_result(selected_tables, pk_map, it_df):
    """ 
    ...
    with tab2:
        ...
        st.dataframe(selected_tables_df, hide_index=True, width=1000, height=500)
...
def main():
    ...
    try:
        ...
        # Data Relationship Diagram 생성 버튼
