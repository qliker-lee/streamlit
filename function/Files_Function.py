...
def load_yaml():
    ...
    yaml_path = 'yaml'
    yaml_file_name = 'QDQM2.0.yaml'

    file_path = os.path.join(CURRENT_DIR_PATH, yaml_path, yaml_file_name)
    try:
...
def Files_Basic_Info_Selection(df):
    ...
    numeric_columns = ['RecordCnt', 'ColumnCnt', 'IsNull', 'IsUnique', 'UnicodeCnt', 'BelowLCL', 
                      'UpperUCL', 'BrokenKoreanCnt', 'ChineseCnt', 'SpecialCnt', 'NullCnt']
    ...
    grouped = st.data_editor(
        ...
        column_config={
            ...
            "NullCnt": st.column_config.NumberColumn("Has Null #", help="NULL 포함 컬럼 수", format="%d"),
...
def Files_Integrity_Info(df):  # 선택한 파일의 모든 무결성 분석
    ...
    columns = ['Check', '주의', 'FileName', 'RecordCnt', 'ColumnCnt', '유니코드', '한글미완성', '하한선미만', 
               '상한선이상', '참조무결성', '일자(문자)점검', '길이점검', 'NullCnt'] 
    ...
    grouped = st.data_editor(
        ...
        column_config={
            ...
            "RecordCnt": st.column_config.NumberColumn("Row #", help="전체 행 수"),
            "ColumnCnt": st.column_config.NumberColumn("Columns #", help="컬럼 수"),
            "NullCnt": st.column_config.NumberColumn("Null #", help="NULL 컬럼 수"),
