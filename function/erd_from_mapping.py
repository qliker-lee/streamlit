...
# -*- coding: utf-8 -*-
"""
ERD 생성 (CodeMapping 결과 기반, 단일 DataFrame 입력)
...
def _build_graph_data(
    ...
    if missing:
        raise ValueError(f"[ERD] 필요한 컬럼 누락: {sorted(missing)}")
...
def _render_erd(
    ...
    dot = Digraph(comment="CodeMapping ERD", encoding="utf-8")
...
def Display_ERD(
    ...
    *,
    ...
    st.markdown("### ERD (선택한 CodeMapping 기반)")
    ...
    with st.spinner("ERD 생성 중..."):
        ...
        try:
            ...
            st.image(image, caption="CodeMapping 기반 ERD", width=img_width)
