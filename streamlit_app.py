...
def intro_page():
    ...
    with st.expander("✅ 시나리오: 특정 컬럼/구조 변경 시 영향도 파악", expanded=True):
        ...
        sc1.error("**Before**: 배포 후 리포트가 깨진 뒤에야 원인 파악 (보수적 운영)")
        sc2.success("**After**: 변경 전 연관 관계 즉시 확인, 리스크 사전 제거")
    
    with st.expander("✅ 시나리오: 신규 인력 온보딩 및 인수인계"):
        sc3, sc4 = st.columns(2)
        sc3.error("**Before**: 문서 중심 설명, 구조 이해까지 수주 소요")
        sc4.success("**After**: ERD 기반 시각화로 단기간 업무 투입 가능 (기간 50% 단축)")
