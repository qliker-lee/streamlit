...
# PK 탐색 시 최대 컬럼 조합 수 (성능을 위해 제한)
MAX_PK_COMBINATION = 20
...
class DataAnalyzer:
    ...
    def export_results_to_csv(self):
        ...
        for table, pk_tuple in self.table_pks.items():
            ...
            pk_data.append([table, pk_str])
        pk_df = pd.DataFrame(pk_data, columns=['테이블명', 'PK 컬럼'])
        pk_output_file = self.output_path / "DS_PK_Discovery_Result.csv"
        ...
        for child_table, fk_relations in self.table_fks.items():
            for rel in fk_relations:
                ...
                fk_data.append([rel['referencing_table'], rel['referenced_table'], fk_cols_str])
        fk_df = pd.DataFrame(fk_data, columns=['Child Table', 'Parent Table', 'FK Columns'])
        fk_output_file = self.output_path / "DS_FK_Discovery_Result.csv"
    ...
    def export_integrated_results_to_csv(self):
        """
        FileName | Column | PK | FK | FK Files | File Count
        """
        integrated_data = []
        ...
        integrated_output_file = self.output_path / "DS_Integrated_PK_FK_Result.csv"
